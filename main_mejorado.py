...
class PodoscopioApp(ctk.CTk):
    ...
    def crear_tarjeta_estudio(self, master, estudio):
        ...
        ModernButton(
            ...
            fg_color="#0f766e",
            hover_color="#115e59",
            command=lambda: self.exportar_pedido_taller(estudio_id)
        ).pack(side="left", padx=3)
...
"""
    ...
    def procesar_imagenes(self, img_izq, img_der):
        ...
        try:
            ...
            # Corregir orientacion segun setup fisico del scanner:
            # - Pie izquierdo: capturado rotado 180 grados (dedos hacia abajo)
            # - Pie derecho: capturado espejado horizontalmente
            try:
                pil_izq = Image.open(img_izq)
                pil_izq_corr = pil_izq.transpose(Image.ROTATE_180).transpose(Image.FLIP_LEFT_RIGHT)
                temp_izq_corr = str(get_temp_file_path("temp_izq_corr", ".png"))
                pil_izq_corr.save(temp_izq_corr)
                img_izq = temp_izq_corr
            except Exception as e:
                self.logger.warning("No se pudo corregir pie izquierdo: %s", e)

            try:
                pil_der = Image.open(img_der)
                pil_der_corr = pil_der.transpose(Image.FLIP_LEFT_RIGHT)
                temp_der_corr = str(get_temp_file_path("temp_der_corr", ".png"))
                pil_der_corr.save(temp_der_corr)
                img_der = temp_der_corr
            except Exception as e:
                self.logger.warning("No se pudo corregir pie derecho: %s", e)
    ...
    def exportar_pedido_taller(self, estudio_id):
        ...
        try:
            ...
            if ReportService.generar_pedido_taller(self.paciente_actual, informe, ruta_pdf, datos_taller):
                ...
                messagebox.showerror("Error", "No se pudo generar el pedido a taller")
        except Exception as e:
            self.logger.exception("Error exportando pedido a taller")
            messagebox.showerror("Error", f"Error generando pedido a taller: {e}")
